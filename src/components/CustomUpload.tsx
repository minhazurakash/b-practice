...
const CustomUpload: React.FC<IUploadProps> = ({
  ...
  onRemove,
  showUploadList = true,
  acceptedType = ["jpeg", "jpg", "png", "svg", "webp"],
  sizeLimit = 2,
  body,
}) => {
  const [messageApi, messageCtx] = message.useMessage();
  const [previewImage, setPreviewImage] = useState<string>("");
  const purifiedFileList = currentFileList?.filter(
  ...
  return (
    <React.Fragment>
      ...
      <Upload
        ...
        action={`https://vt-bangladesh.uniclienttechnologies.com/api/v1/internal/files/new`}
        ...
        onRemove={(v) => {
          ...
          const filteredItems = purifiedFileList?.filter(
            (item) => item !== v?.response?.data?.[0]?.url
          );
          const purifiedItems = filteredItems?.filter(
            (item) => item !== v?.url
          );
